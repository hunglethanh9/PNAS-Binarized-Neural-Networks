...
class StateSpace:
    ...
    def __init__(self, B, operators, input_lookback_depth=-1, input_lookforward_depth=None):
        ...
        if operators is None:
            self.operators = ['3x3 bconv','5x5 bconv','7x7 bconv', '1x7-7x1 bconv',
              '3x3 maxpool']
        else:
    ...
    def _add_state(self, name, values):
        ...
        # print("-----Metadata------")
        # print(metadata)
        # print("embedding_max: ", self.embedding_max)
    ...
    def print_total_models(self, K):
        ''' Compute the total number of models to generate and train. Compute level1 separately and then the other levels :D '''
...
class Encoder:
    ...
    def __init__(self, policy_session, state_space, exp_name,
                 B=5, K=256,
                 train_iterations=10,
                 reg_param=0.001,
                 controller_cells=32,
    ...
    def build_policy_network(self):
        with self.policy_session.as_default():
            ...
            timestr = time.strftime(self.exp_name + "-%m-%d-%H-%M")
            ...
            if self.restore_controller:
                path = tf.train.latest_checkpoint(weights_path)
            ...
            for dataset_id in range(self.b_):
                ...
                for id, (child, score) in enumerate(zip(children[ids], scores[ids])):
                    ...
                    with self.policy_session.as_default():
                        ...
                        self.saver.save(self.policy_session, save_path= weights_path +'controller.ckpt', global_step=self.global_step)
        ...
        if self.b_ + 1 <= self.B:
            with self.policy_session.as_default():
                ...
                for i, intermediate_child in enumerate(self.state_space.prepare_intermediate_children(self.b_)):
                    ...
                    scores_path = 'scores/' + self.exp_name + '/'
                    if not os.path.exists(scores_path):
                        os.makedirs(scores_path)

                    with open(scores_path + 'scores_%d.csv' % (self.b_), mode='a+', newline='') as f:
                        writer = csv.writer(f)
                        data = [score]
